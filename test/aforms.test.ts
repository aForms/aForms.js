import {AFormModel, AFormModelClass} from "../src";
const jsdom = require("jsdom");
const { JSDOM } = jsdom;
const { window } = (new JSDOM(`<div id="test-div"></div>`));
 ...
 *
 * Test AClass initialization
 */

const initializedTestDiv = window.document.getElementById('test-div');

const sampleFormModal: AFormModel = {
  ...
  "components": [{
    ...
    "values": [{"value": "excellent", "label": "Excellent"}, {"value": "great", "label": "Great"}, {
      "value": "good",
      "label": "Good"
  ...
  "submissionAccess": [{"type": "create_all", "roles": []}, {"type": "read_all", "roles": []}, {
    "type": "update_all",
    "roles": []
  }, {"type": "delete_all", "roles": []}, {
    "type": "create_own",
    "roles": ["5692b920d1028f01000407e6"]
  }, {"type": "read_own", "roles": []}, {"type": "update_own", "roles": []}, {"type": "delete_own", "roles": []}],
  "access": [{
    "type": "read_all",
...
describe('initialize forms library', () => {
  it("fail if div element and aForm modal are not provided",  () => {
    function createForm() {
      aFormClass = new AFormModelClass()
    }
    expect(createForm).toThrowError(new Error("A Form json and div element location are required to initialize library."))
  })

  it('rendered form is instance of aFormClass', () => {
    let aFormModelClass = new AFormModelClass(sampleFormModal, initializedTestDiv);
    aFormModelClass.renderModel().then(value => {
      aFormClass = value;
      expect(value).toBeInstanceOf(AFormModelClass)
    }).catch(e => {
      expect(e).toEqual("Display type not available.")
    })
  })
})
